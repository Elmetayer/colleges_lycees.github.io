...
# données collèges
df_college_new <- read.csv2("data_raw/fr-en-indicateurs-valeur-ajoutee-colleges.csv") %>% 
  left_join(
    df_geo,
    by = c("UAI" = "numero_uai")
  ) %>% 
  filter(!is.na(Taux.de.réussite.G)) %>% 
  mutate(
    type = "collège",
    Secteur = ifelse(Secteur == "PU", "public", "privé"),
    Taux.de.mentions.Gnle = Nb.mentions.global.G / Nb.candidats.G,
    Taux.de.mentions.TB.Gnle = Nb.mentions.TB.G / Nb.candidats.G,
    score = rowSums(across(c("Taux.de.réussite.G", "Taux.de.mentions.Gnle", "Taux.de.mentions.TB.Gnle"),
                           ~(as.numeric(.x))))) %>% 
  group_by(Session) %>% 
  arrange(desc(score)) %>% 
  mutate(
...
df_college_old <- read.csv2("data_raw/fr-en-dnb-par-etablissement.csv") %>% 
  ...
  mutate(
    ...
    score = Admis/Presents + 1 - Admis.sans.mention/Presents + Admis.Mention.très.bien/Presents) %>% 
  group_by(Session) %>% 
  arrange(desc(score)) %>% 
  mutate(
