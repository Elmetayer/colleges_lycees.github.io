...
df_geo <- read.csv2("data/fr-en-adresse-et-geolocalisation-etablissements-premier-et-second-degre.csv") %>% 
  select(cols_geo) %>% 
  mutate(
...
df_lycee <- read.csv2("data/fr-en-indicateurs-de-resultat-des-lycees.csv") %>% 
  select(cols_lycee) %>% 
  left_join(
    df_geo,
    by = c("UAI" = "numero_uai")
...
df_college <- read.csv2("data/fr-en-indicateurs-valeur-ajoutee-colleges.csv") %>% 
  select(cols_college) %>% 
  left_join(
    df_geo,
    by = c("UAI" = "numero_uai")
...
df_ecole <- read.csv2("data/fr-en-adresse-et-geolocalisation-etablissements-premier-et-second-degre.csv") %>% 
  filter(
...
df_carte <- rbind(
  df_college %>% 
    mutate(
      type = "collège",
      nb = nb_colleges) %>% 
    select(latitude, longitude, Nom.de.l.établissement, type, Secteur, Commune, code_departement, score, classement, nb) %>% 
    rename(Etablissement = Nom.de.l.établissement),
  df_lycee %>% 
    mutate(
      type = "lycée",
      nb = nb_lycees) %>% 
    select(latitude, longitude, Etablissement, type, Secteur, Commune, code_departement, score, classement, nb)) %>% 
  mutate(
    score = round(score, 2),
